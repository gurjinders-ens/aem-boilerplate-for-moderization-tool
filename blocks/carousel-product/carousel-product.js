...
function createSlide(row, slideIndex, carouselId) {
  ...
  // Mark up the content cell: product title, optional "Best Seller" badge, and price.
  const content = slide.querySelector('.carousel-product-slide-content');
  if (content) {
    const paragraphs = [...content.querySelectorAll(':scope > p')];
    paragraphs.forEach((p) => {
      const text = p.textContent.trim();
      if (p.querySelector('a, strong')) {
        p.classList.add('carousel-product-slide-title');
      } else if (/best\s*seller/i.test(text)) {
        p.classList.add('carousel-product-slide-badge');
        // Move the badge to the image cell so it overlays the product image.
        const image = slide.querySelector('.carousel-product-slide-image');
        if (image) image.append(p);
      } else if (/^[$ۣ]?\s*[\d.,]+$/.test(text)) {
        p.classList.add('carousel-product-slide-price');
      }
    });
  }
